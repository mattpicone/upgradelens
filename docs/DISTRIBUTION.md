...
|---|---|---|
| GitHub public repo | DONE | https://github.com/mattpicone/upgradelens. Topics include `mcp`, `mcp-server`, `gemini-cli-extension`, and `agent-plugins`. The repository contract is now v0.3.0 |
| Official MCP Registry (registry.modelcontextprotocol.io) | PENDING RELEASE | v0.3.0 metadata is canonical in `src/contract.ts` and `server.json`; publish and verify the immutable version through the OIDC workflow before claiming it is indexed |
...
1. Publish the immutable v0.3.0 record via `Publish to MCP Registry` OIDC workflow, then verify the exact public API response.
